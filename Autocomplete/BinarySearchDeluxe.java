...
public class BinarySearchDeluxe {
    ...
    public static <Key> int firstIndexOf(Key[] a, Key key, Comparator<Key> c) {
        if (a == null || key == null || c == null) {
            throw new NullPointerException("a, key, or c is null");  // a key or c is null
        }

        int lo = 0;  // lo
        int hi = a.length - 1;  // hi
        int index = -1;  // index

        while (lo <= hi) {  // as long as lo less equal hi
            int mid = lo + (hi - lo) / 2;
            int cmp = c.compare(key, a[mid]);
            if (cmp < 0) {  // if cmp less 0
                hi = mid - 1;
            } else if (cmp > 0) {  // if cmp bigger 0
                lo = mid + 1;
            } else {
                index = mid;  // index = mid
                hi = mid - 1;
    ...
    public static <Key> int lastIndexOf(Key[] a, Key key, Comparator<Key> c) {
        ...
        while (lo <= hi) {  // lo less equal hi
            int mid = lo + (hi - lo) / 2;
            int cmp = c.compare(key, a[mid]);
            if (cmp < 0) {  // cmp less 0
                hi = mid - 1;
            } else if (cmp > 0) {  // cmp bigger 0
                lo = mid + 1;
            } else {
                index = mid;
                lo = mid + 1;
